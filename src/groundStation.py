...
from src.utils import get_tle_epoch_tai
...
class GroundStation(nn.Module):
    ...
    """
    def __init__(self, lat_deg: float, lon_deg: float, alt_km: float, epoch_tai: float, station_id: str) -> None:
        super().__init__()
        ...
        loc = EarthLocation(lat=lat_deg*u.deg, lon=lon_deg*u.deg, height=alt_km*u.km)
        
        # Convert to km and store as a registered buffer (moves to GPU automatically)
        xyz = np.array(object=[loc.x.to(u.km).value, loc.y.to(u.km).value, loc.z.to(u.km).value])
        self.register_buffer(name='pos_ecef', tensor=torch.tensor(data=xyz, dtype=torch.float64))
