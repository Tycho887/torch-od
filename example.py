...
def compiled_forward_model(
    ...
    pos_sat, vel_sat = compiled_propagate(
        tsinces,
        bstar=x[0].unsqueeze(0), ndot=x[1].unsqueeze(0), nddot=x[2].unsqueeze(0),
        ecco=x[3].unsqueeze(0), argpo=x[4].unsqueeze(0), inclo=x[5].unsqueeze(0),
        mo=x[6].unsqueeze(0), no_kozai=x[7].unsqueeze(0), nodeo=x[8].unsqueeze(0)
    )
...
def simulate_and_recover_compiled():
    ...
    print("-" * 55)
    print(f"{'Mean Anomaly':<15} | {mo_init:<10.5f} | {mo_true:<10.5f} | {mo_opt:<10.5f}")
