...
def load_gmat_csv_block(file_path, tle_epoch_unix, block_sec) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """
    Reads GMAT CSV (TAIModJulian, X, Y, Z, VX, VY, VZ), 
    aligns timestamps to UTC, and filters data around the TLE epoch.
    """
    # Read the 7 columns, skipping the header
    ...
    # Extract aligned time, position (cols 1, 2, 3), and velocity (cols 4, 5, 6)
    t_gps_np = df_block['unix'].values
    gps_pos_np = df_block[[1, 2, 3]].values 
    gps_vel_np = df_block[[4, 5, 6]].values 

    # Convert NumPy arrays to PyTorch Tensors
    t_gps = torch.from_numpy(np.copy(t_gps_np)).to(dtype=torch.float64) #
    gps_pos = torch.from_numpy(np.copy(gps_pos_np)).to(dtype=torch.float64) #
    gps_vel = torch.from_numpy(np.copy(gps_vel_np)).to(dtype=torch.float64) #
    
    return t_gps, gps_pos, gps_vel

def load_gmat_csv_block_legacy(file_path, tle_epoch_unix, block_sec) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """
