...
def compute_cca_step(
    ...
    """
    Computes the CCA update and covariance using a Truncated SVD pseudoinverse 
    to handle poor observability and collinear parameters.
    """
    # 1. Partitioning the Jacobian
    H_x = H_total[:, estimate_mask]
    H_c = H_total[:, consider_mask]

    # 2. Create the weight scalar/vector
    W = 1.0 / (sigma_obs**2)
    H_x_W = H_x.T * W

    # 3. Accumulate Information Matrix (M_xx) and Right-Hand Side (N_x)
    M_xx = H_x_W @ H_x
    M_xx += P_x_inv
    ...
    # 4. Truncated SVD Pseudoinverse
    # Replaces torch.linalg.cholesky to filter out collinear singular values
    M_xx_pinv = torch.linalg.pinv(M_xx, rcond=rcond)

    # Solve for the state update (dx)
    dx = M_xx_pinv @ N_x

    # 5. Consider Covariance Step
