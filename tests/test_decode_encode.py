...
def test_single_tle_io(tle_str, sat_num, epoch):
    ...
    assert tensor_orig.shape == (9,), (
        "Decoded single tensor should be 1D with 9 elements"
    )

    # 2. Encode back to string
    encoded_lines = tle_encode(
        bstar=tensor_orig[0].item(),
        ndot=tensor_orig[1].item(),
        nddot=tensor_orig[2].item(),
        ecco=tensor_orig[3].item(),
        argpo=tensor_orig[4].item(),
        inclo=tensor_orig[5].item(),
        mo=tensor_orig[6].item(),
        no_kozai=tensor_orig[7].item(),
        nodeo=tensor_orig[8].item(),
        sat_num=sat_num,
        epoch=epoch,
    )
    ...
    assert torch.allclose(tensor_orig, tensor_recon, atol=1e-8), (
        "Data lost during encode/decode round-trip"
    )
...
def test_batch_tle_io():
    ...
    assert torch.allclose(tensor_batch, tensor_batch_recon, atol=1e-8), (
        "Data lost in batch round-trip"
    )
