...
def benchmark_custom(tensor_batch, tsinces):
    ...
    pos, vel = compiled_propagate(
        tsinces,
        bstar=tensor_batch[:, 0].unsqueeze(1), ndot=tensor_batch[:, 1].unsqueeze(1),
        nddot=tensor_batch[:, 2].unsqueeze(1), ecco=tensor_batch[:, 3].unsqueeze(1),
        argpo=tensor_batch[:, 4].unsqueeze(1), inclo=tensor_batch[:, 5].unsqueeze(1),
        mo=tensor_batch[:, 6].unsqueeze(1), no_kozai=tensor_batch[:, 7].unsqueeze(1),
...
def run_benchmarks():
    ...
    print(f"{'Mode':<15} | {'Timesteps':<10} | {'Custom (s)':<12} | {'dSGP4 (s)':<12} | {'Speedup':<10}")
    print("-" * 70)
    ...
    for steps in timesteps_to_test:
        ...
        print(f"{'Single TLE':<15} | {steps:<10} | {custom_time:<12.5f} | {dsgp4_time:<12.5f} | {speedup:.1f}x")
        ...
        print(f"{'Batch (2 TLEs)':<15} | {steps:<10} | {custom_time_batch:<12.5f} | {dsgp4_str:<12} | {speedup_str:<10}")
